...
from domain.interfaces.delete_product_interface import DeleteProductInterface

class ProductController:
    def __init__(
            self,
            create_product_use_case: CreateProductInterface,
            list_product_use_case: ListProductInterface,
            update_product_use_case: UpdateProductInterface,
            delete_product_use_case: DeleteProductInterface
        ):
        ...
        self.router.add_api_route("/products/{id}", self.delete, methods=["DELETE"])
    ...
    async def delete(self, id: int = Path(..., title="Product ID", description="The ID of the product to delete")) -> None:
        self.delete_product_use_case.execute(id)
        return